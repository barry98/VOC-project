...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="Location of files you want to predict on."
    )
    parser.add_argument(
        "--labels",
        default="",
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for file in os.listdir(args.files):
            result, predictions = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode=file[:-4])
            # Save results
            ...
            # Save predictions
            output_test_predictions_file = os.path.join(args.output_dir, str(file[:-4]) +"_predictions.txt")
            with open(output_test_predictions_file, "w") as writer:
                with open(os.path.join(args.data_dir, str(file)), "r") as f:
                    example_id = 0
                    for line in f:
